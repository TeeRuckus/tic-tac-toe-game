...
int main(void)
{
    ...
    /*TEST 1: all file interface testing*/
    printf(BLUE"TEST 1: "RESET_COLOR);
    fileInterfaceTest(gameSettings);

    /*TEST 2: link list testing ensuring all linked list methods work 
     * correctly*/
    printf(BLUE"TEST 2: "RESET_COLOR);
    linkedListTest();
    
    /*TEST 3: visual test. Testing if the logs display the correct manner*/
    printf(BLUE"TEST 3: "RESET_COLOR);
    testLog = displayLogTest();
    
    /*TEST 4: manual testing of the game*/
...
/*PURPOSE: to test possible files the file interface may be required to read in*/
void fileInterfaceTest(int *retSettings)
{
    ...
    /*testing reading a negative setting from a file*/
...
LinkedList* displayLogTest()
{
    ...
    *numMoves = 0;
    ...
    /*displaying the log after the simulation of game play. Compare the
     * actual moves fromt the simulation to the displayed values*/
    ...
    *numMoves = 0;
    ...
    /*testing if the information from game one will persist*/
...
{   
    /*this test is meant to be done in conjuction with the display log test,
     * compare if the information written to the file is the same as the
     * information displated to the terminal*/
    Status result;
    ...
    /*check if the file saved in the following format 
     * MNK_<M>-<N>-<K>_<HOUR>-<MIN>_<DAY>-<MONTH>.log, the time is meant to
     * be in 24 hour time*/
