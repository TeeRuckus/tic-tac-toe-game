...
void changeGameSettings(int *inGameSettings)
{
    ...
    do
    {
        ...
        /*I a setting a limit on how big the baord can be. Otherwise valgrind
        will throw this error
        ==23401== Warning: set address range perms: large range [0x59e43040, 
        0x3c595e158) (undefined) as we're trying to create memory which way
        too large*/
        if(numReturned != 3)
        {
            printf(RED"ERROR: invalid argument arrangment. Enter options again:"
            RESET_COLOR);
            stop = FALSE;
            while((getchar()) != '\n');
        }
        else if(stop == FALSE)
        {
            ...
            " than or less than %d x %d: "RESET_COLOR, MAX_ROWS, MAX_COLUMNS);
            while((getchar()) != '\n');
        }
    }while(!stop);
...
Boolean validateUserSettings(int mSetting, int nSetting, int kSetting) 
{
    Boolean isValid;
    isValid = FALSE;
    if( (mSetting > 0 && mSetting < MAX_ROWS) && (nSetting > 0 && nSetting 
    < MAX_COLUMNS) && (kSetting > 0) && (kSetting < MAX_TILES_WIN))
    {
        isValid = TRUE;
    }
